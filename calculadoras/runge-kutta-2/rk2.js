const App = {
  ...
  limpiarTodo() {
    if (confirm('øEst·s seguro de limpiar todos los datos?')) {
      ...
      document.getElementById('graficoRK2').innerHTML = '';
  ...
  calcular() {
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        // C·lculo de k2
        const k2 = this.evaluarFuncion(expresion, x + h, y + h * k1);

        // C·lculo del incremento
        const deltaY = (h / 2) * (k1 + k2);
      ...
      alert(`C·lculo completado exitosamente: ${n} puntos generados mediante Runge-Kutta orden 2`);
  ...
  mostrarDesarrollo() {
    ...
    let latex = `
      <div class="paso-desarrollo">
        <h4>Paso 1: MÈtodo Runge-Kutta Orden 2 - FÛrmula General</h4>
        <p>El mÈtodo de Runge-Kutta orden 2 es un mÈtodo numÈrico de segundo orden que proporciona mayor precisiÛn que el mÈtodo de Euler. Utiliza dos evaluaciones de la funciÛn por paso:</p>
        <p>$$k_1 = f(x_n, y_n)$$</p>
        <p>$$k_2 = f(x_n + h, y_n + h \\cdot k_1)$$</p>
        <p>$$y_{n+1} = y_n + \\frac{h}{2}(k_1 + k_2)$$</p>
        <p>$$x_{n+1} = x_n + h$$</p>
        <p style="margin-top: 12px; font-size: 0.95em; color: var(--gris-texto);">Este mÈtodo tambiÈn se conoce como el mÈtodo de Heun o mÈtodo mejorado de Euler</p>
      </div>
      ...
      <div class="paso-desarrollo">
        ...
        <p style="margin-bottom: 15px; color: var(--gris-texto);">Se aplica la fÛrmula de Runge-Kutta orden 2 calculando kÅ y kÇ en cada iteraciÛn:</p>
    `;
      ...
      if (idx < r.puntos.length - 1) {
        ...
        latex += `
          <p style="margin: 12px 0; padding: 12px; background: #f0f9ff; border-left: 4px solid #1e40af; border-radius: 4px;">
            ...
            $$k_2 = f(${this.formatear(p.x)} + ${this.formatear(r.h)}, ${this.formatear(p.y)} + ${this.formatear(r.h)} \\times ${this.formatear(p.k1)}) = ${this.formatear(p.k2)}$$<br>
            $$y_{${idx + 1}} = ${this.formatear(p.y)} + \\frac{${this.formatear(r.h)}}{2}(${this.formatear(p.k1)} + ${this.formatear(p.k2)}) = ${this.formatear(p_siguiente.y)}$$<br>
    ...
    latex += `
      ...
      <div class="resultado-final">
        ...
        <p>Se han calculado ${r.n} puntos de la soluciÛn aproximada usando el MÈtodo de Runge-Kutta Orden 2 (Heun) con h = ${this.formatear(r.h)}</p>
      </div>
    `;
  ...
  graficar() {
    ...
    const titulo = document.getElementById('tituloExperimento').value ||
                   `Runge-Kutta Orden 2: dy/dx = ${r.expresion}`;
    ...
    Plotly.newPlot('graficoRK2', [traza], layout, config);
  },
